  ...
  `}}function getControlGap(settings){return settings.rowHeight/6}function Bar(settings){let dataDateX=this.timeScale.dateToPx(this.schedule.dataDate),y0=(settings.rowHeight-settings.barHeight)/2,todayX=this.timeScale.dateToPx(new Date(new Date().setHours(0,0,0,0))),scale=this.timeScale,bars=this.schedule.items.map((v3,i5)=>{let id="bar_"+v3.id,handler=()=>{if(this.suppressClick)return;let ev=new CustomEvent("item-click",{detail:v3});this.dispatchEvent(ev)},x2=scale.dateToPx(v3.earlyStart),y3=y0+i5*settings.rowHeight,grHeight=settings.barHeight/3;v3.type==="group"&&(y3=(settings.rowHeight-grHeight)/2+i5*settings.rowHeight);let cy=settings.barHeight/2+1;if(v3.type==="milestone")return renderMilestone.bind(this)(x2,y3,settings.barHeight,handler,id,v3);let w1=scale.pxForTimeSpan(v3.earlyStart,v3.earlyFinish);if(isNaN(w1))throw Error("Invalid argument: v: "+v3);let progressDate=(0,import_dayjs4.default)(v3.earlyStart).add(Math.min(v3.progressDays,v3.duration),"days").toDate(),w2=scale.pxForTimeSpan(v3.earlyStart,progressDate),barCss2="gantt-bar";barCss2+=v3.type==="group"?" group":"";let controlRadius=settings.rowHeight/6,controlGap=getControlGap(settings),controlBorder=1,controlsOffset=controlRadius*2+controlGap+controlBorder*2,isDelayed=v3.type==="activity"&&v3.delayDays>0,borderCssClass=v3.crit?"bar-inner-border crit":"bar-inner-border";isDelayed&&(borderCssClass+=" delayed");let barBorder=b2`
    ...
    `,backBar=b2`
      ...
      <svg 
        ...
        data-item-id=${v3.id}        
        class=${barCss2}
        style=${v3.color?`--gantt-bar-group-back-fill: ${v3.color}; --gantt-bar-inner-border-color:  ${v3.color};`:""}
