    ...
    `,backBar=b2`
      ...
      `,frontBar=w2>1e-6?b2`
          <rect
            ...
            class="front"
            @click=${handler}
            />`:null,controls=b2``;return v3.type==="activity"&&(controls=b2`
        <circle
...
`;function configureAddLink(){let svg2=this.shadowRoot.getElementById("gantt");function isStart(el){return el.classList.contains("ctl-start")}function isFinish(el){return el.classList.contains("ctl-finish")}let addLink=(s5,e6)=>{let sid=s5.dataset.id,eid=e6.dataset.id;if(sid===eid)return;let startNode=this.schedule.itemsIndex.get(sid),endNode=this.schedule.itemsIndex.get(eid),startType=isStart(s5)?"S":"F",endType=isStart(e6)?"S":"F";startNode.type==="milestone"&&(startType="F"),endNode.type==="milestone"&&(endType="S");let link={predecessor:sid,successor:eid,type:`${startType}${endType}`,lag:0};if(this.schedule.dependencies.find(x2=>x2.predecessor===sid&&x2.successor===eid))return;let isCanceled=!1,evArgs={link,cancel:()=>{isCanceled=!0}},ev=new CustomEvent("before-link-added",{detail:evArgs});this.dispatchEvent(ev),isCanceled||(this.schedule.dependencies.push(link),this.requestUpdate())},NS="http://www.w3.org/2000/svg",moving=!1,start=null,line=null,hideFinish=!1,getControlX=ctl=>ctl.parentElement.x.baseVal.value+ctl.cx.baseVal.value,getControlY=ctl=>ctl.parentElement.y.baseVal.value+ctl.cy.baseVal.value;svg2.addEventListener("mousedown",e6=>{if(!isStart(e6.target)&&!isFinish(e6.target))return;e6.preventDefault(),start=e6.target;let isDependencyStart=isStart(start);hideFinish=this.settings.disableSF&&isDependencyStart,hideFinish&&this.shadowRoot.querySelectorAll(".activity.ctl-finish").forEach(e7=>e7.toggleAttribute("disabled",!0)),(hideFinish?this.shadowRoot.querySelectorAll(".activity.ctl-start"):this.shadowRoot.querySelectorAll(".activity.ctl-start,.activity.ctl-finish")).forEach(elem=>{elem.toggleAttribute("active",!0)}),moving=!0,line=document.createElementNS(NS,"line");let x2=getControlX(start).toString(),y3=getControlY(start).toString();line.setAttribute("x1",x2),line.setAttribute("y1",y3),line.setAttribute("x2",x2),line.setAttribute("y2",y3),line.classList.add("add-link-line"),svg2.appendChild(line)}),svg2.addEventListener("mousemove",e6=>{let target=e6.target;if(moving)if(e6.preventDefault(),isStart(target)||isFinish(target)){let x2=getControlX(target).toString(),y3=getControlY(target).toString();line.setAttribute("x2",x2),line.setAttribute("y2",y3)}else{let x2=e6.clientX,y3=e6.clientY,rect=svg2.getBoundingClientRect();line.setAttribute("x2",(x2-rect.left).toString()),line.setAttribute("y2",(y3-rect.top).toString())}});let resetMovingControls=e6=>{moving&&(e6.preventDefault(),e6.stopPropagation(),this.shadowRoot.querySelectorAll(".ctl-start,.ctl-finish").forEach(elem=>{elem.toggleAttribute("active",!1),elem.toggleAttribute("disabled",!1)}),moving=!1,line&&(svg2.removeChild(line),line=null))};svg2.addEventListener("mouseup",e6=>{resetMovingControls(e6);let target=e6.target,isCtrl=isStart(target)||isFinish(target);if(hideFinish&&isFinish(target)){start=null,hideFinish=!1;return}start&&isCtrl&&addLink(start,target),start=null,hideFinish=!1}),this.addEventListener("mouseout",e6=>{resetMovingControls(e6),start=null,hideFinish=!1})}var svg,resizeItem=(itm,diffMs,resizeStart)=>{if(!svg)return;let diffDays=Math.abs(Math.round(diffMs/MsInDAY));resizeStart?diffMs>0?itm.duration=Math.max(itm.duration-diffDays,1):itm.duration+=diffDays:diffMs<0?itm.duration=Math.max(itm.duration-diffDays,1):itm.duration+=diffDays};function configureResizeItem(){svg=this.shadowRoot.getElementById("gantt");let moving=!1,movingStarted=!1,barSvg,itemId,item,resizeStart=!1,initialX;function isResizeControl(e6){let p4=e6.composedPath(),_resizeStart=!1,_resizeEnd=!1;for(let x2 of p4)if(_resizeStart=x2.classList?.contains("ctl-resize-start"),_resizeEnd=x2.classList?.contains("ctl-resize-end"),_resizeStart||_resizeEnd)break;return _resizeStart||_resizeEnd?(resizeStart=_resizeStart,!0):!1}let resetMovement=()=>{resizeStart=!1,moving=!1,barSvg=void 0,initialX=void 0,item=void 0,movingStarted=!1,this.suppressClick},cancelEvent=e6=>{e6.preventDefault(),e6.stopPropagation(),e6.stopImmediatePropagation()},onMouseDown=e6=>{!isResizeControl(e6)||moving||(movingStarted=!0,initialX=e6.clientX,barSvg=e6.composedPath().find(x2=>x2.classList.contains("gantt-bar")),itemId=barSvg.dataset.itemId,itemId&&(cancelEvent(e6),item=this.schedule.itemsIndex.get(itemId)))},onMouseMove=async e6=>{if(!movingStarted)return;cancelEvent(e6),moving=!0;let dir=e6.movementX>0?1:-1,diff=Math.abs(initialX-e6.clientX)*this.timeScale.msPerPx*dir;diff!==0&&(Math.abs(diff)<MsInDAY||(resizeItem(item,diff,resizeStart),initialX=e6.clientX,this.requestUpdate()))},onMouseUp=e6=>{if(movingStarted){if(moving){this.___lastMovement=new Date().getTime();let ev=new CustomEvent("item-resized",{detail:item});this.dispatchEvent(ev)}resetMovement(),cancelEvent(e6)}};svg.addEventListener("mousedown",onMouseDown),svg.addEventListener("mousemove",onMouseMove),svg.addEventListener("mouseup",onMouseUp),this.addEventListener("mouseout",()=>{resetMovement()})}var WcGanttSettings=class{constructor(){this.disableSF=!1;this.timeScaleMode="week";this.showDependencies=!0;this.showLabels=!0;this.showCriticalPath=!0;this.startDate=new Date(new Date().setHours(0,0,0,0));this.dataDate=new Date(new Date().setHours(0,0,0,0));this.baselineDate=new Date(new Date().setHours(0,0,0,0))}};var import_dayjs7=__toESM(require_dayjs_min(),1);var import_dayjs6=__toESM(require_dayjs_min(),1);var TreeNode=class _TreeNode{constructor(schedule,id){this.schedule=schedule;this.id=id;this.startDate=this.schedule.itemsIndex.get(this.id).earlyStart;this.endDate=this.schedule.itemsIndex.get(this.id).earlyFinish}get children(){return this.schedule.dependencies.filter(l3=>l3.predecessor===this.id&&(0,import_dayjs6.default)(this.schedule.itemsIndex.get(this.id).earlyFinish).add(l3.lag,"days").toDate().getTime()===this.__getSucDate(l3)).map(x2=>new _TreeNode(this.schedule,x2.successor))}__getSucDate(l3){if(l3.type.endsWith("F"))return this.schedule.itemsIndex.get(l3.successor).earlyFinish.getTime();if(l3.type.endsWith("S"))return this.schedule.itemsIndex.get(l3.successor).earlyStart.getTime()}};function findLongestPath(startItemId,schedule){let longestPathResult={longestDuration:0,path:[]},dfs=(currentNodeId,path,currentDuration)=>{if(currentNodeId===null)return;let currentNode=new TreeNode(schedule,currentNodeId),nodeDuration=currentNode.endDate.getTime()-currentNode.startDate.getTime(),newDuration=currentDuration+nodeDuration;path.push(currentNode.id),!currentNode.children||currentNode.children.length===0?newDuration>longestPathResult.longestDuration&&(longestPathResult.longestDuration=newDuration,longestPathResult.path=[...path]):currentNode.children.forEach(child=>{dfs(child.id,path,newDuration)}),path.pop()};return dfs(startItemId,[],0),longestPathResult.longestDuration=longestPathResult.longestDuration/MsInDAY,longestPathResult}var Item=class{constructor(s5){this.s=s5;this.nested=[];this.id=crypto.randomUUID();this.name=this.id;this.progressDays=0;this.type="activity";this._duration=14;this.crit=!1}get dataDate(){return new Date(this._dataDate??new Date(Math.max(this.s.dataDate?.getTime()??this.s.startDate.getTime())).setHours(0,0,0,0))}set dataDate(v3){v3&&(this._dataDate=new Date(v3.setHours(0,0,0,0)))}get duration(){if(this.type==="milestone")return 0;if(this.type!=="group")return this._duration;if(this.nested.length===0)return 1;let startTimes=[],endTimes=[];for(let i5 of this.nested)startTimes.push(i5.earlyStart.getTime()),endTimes.push(i5.earlyFinish.getTime());let minStart=Math.max(...startTimes),maxEnd=Math.max(...endTimes);return(0,import_dayjs7.default)(maxEnd).diff(minStart,"days")}set duration(v3){this.type!=="group"&&this.type!=="milestone"&&(this._duration=v3)}get lateStart(){return this.earlyStart}get lateFinish(){return this.earlyFinish}__getEarlyStartBasedOnDependency(d3){let pred=this.s.itemsIndex.get(d3.predecessor);if(!pred)return this.defaultStartDate;switch(d3.type){case"FS":return(0,import_dayjs7.default)(pred.earlyFinish).add(d3.lag,"days").toDate();case"FF":{let calculatedDate=(0,import_dayjs7.default)(pred.earlyFinish).add(d3.lag,"days").subtract(this.duration,"days").toDate().getTime();return new Date(Math.max(this.defaultStartDate.getTime(),calculatedDate))}case"SS":return(0,import_dayjs7.default)(pred.earlyStart).add(d3.lag,"days").toDate();case"SF":return(0,import_dayjs7.default)(pred.earlyStart).subtract(d3.lag,"days").subtract(this.duration,"days").toDate()}}get isStarted(){return this.progressDays>0}get defaultStartDate(){return this.isStarted?(0,import_dayjs7.default)(this.dataDate).subtract(this.progressDays,"days").toDate():this.dataDate}__getEarlyStartBasedOnSFDependency(d3){let suc=this.s.itemsIndex.get(d3.successor);return(0,import_dayjs7.default)(suc.earlyFinish).add(d3.lag,"days").toDate()}get earlyStart(){if(this.type==="group")return new Date(Math.min(...this.nested.map(x2=>x2.earlyStart.getTime())));let predDeps=this.s.dependencies.filter(x2=>x2.successor===this.id&&x2.type!=="SF"),succeedingDep=this.s.dependencies.filter(x2=>x2.predecessor===this.id&&x2.type==="SF");if(predDeps.length===0&&succeedingDep.length===0)return this.defaultStartDate;let earlyStarts=predDeps.map(pd=>this.__getEarlyStartBasedOnDependency(pd).getTime()),sfEarlyStarts=succeedingDep.map(sd=>this.__getEarlyStartBasedOnSFDependency(sd).getTime());return earlyStarts.push(...sfEarlyStarts),new Date(Math.max(...earlyStarts))}get earlyFinish(){return this.type==="group"?new Date(Math.max(...this.nested.map(x2=>x2.earlyFinish.getTime()))):this.type==="milestone"?this.earlyStart:(0,import_dayjs7.default)(this.earlyStart).add(this.duration,"days").toDate()}},Schedule=class{constructor(dataDate,items,dependencies){this.dataDate=new Date;this.__nonGroups=["activity","buffer","milestone"];this.dataDate=new Date(dataDate.setHours(0,0,0,0)),this.itemsIndex=new Map,this.items=this._flattenItems(items),this.items.forEach(x2=>{this.itemsIndex.set(x2.id,x2)}),this.dependencies=dependencies}get startDate(){return new Date(Math.min(...this.items.map(x2=>x2.earlyStart.getTime())))}get endDate(){return new Date(Math.max(...this.items.map(x2=>x2.earlyFinish.getTime())))}get durationDays(){return(0,import_dayjs7.default)(this.endDate).diff(this.startDate,"days")}_flattenItems(items){let flatArray=[];for(let item of items){let i5=new Item(this);i5.id=item.id,i5.name=item.name,flatArray.push(i5),item.type==="group"||item.nested&&Array.isArray(item.nested)&&!this.__nonGroups.includes(item.type)?(i5.nested=[],i5.type="group",i5.nested=this._flattenItems(item.nested??[]),flatArray.push(...i5.nested)):(i5.dataDate=item.dataDate,i5.duration=item.duration!=null&&item.duration>0?item.duration:i5.duration,i5.progressDays=item.progressDays??0,i5.type=this.__nonGroups.includes(item.type)?item.type:"activity")}return flatArray}updateCriticalPath(){let nonGroups=this.items.filter(x2=>x2.type!=="group"),minItems=nonGroups.filter(x2=>x2.earlyStart.getTime()===Math.min(...nonGroups.map(r7=>r7.earlyStart.getTime()))),maxItems=nonGroups.filter(x2=>x2.earlyFinish.getTime()===Math.max(...nonGroups.map(r7=>r7.earlyFinish.getTime()))),critItems=[];for(let x2 of minItems){let lp=findLongestPath(x2.id,this);maxItems.map(x3=>x3.id).includes(lp.path[lp.path.length-1])&&critItems.push(...lp.path)}for(let i5 of this.items)i5.crit=critItems.includes(i5.id)}};var TimeScale=class{get start(){return this._start}set start(v3){this._start=new Date(v3.setHours(0,0,0,0))}get startMs(){return this.start.getTime()}get pxPerWeek(){return this.pxPerDay*7}get pxPerDay(){return Math.round(22/{month:4,week:2,day:1}[this.viewMode])}get msPerPx(){return 864e5/this.pxPerDay}get end(){return this._end}set end(v3){this._end=new Date(v3.setHours(0,0,0,0))}get totalDays(){return Math.round(Math.abs((this.startMs-this.endMs)/864e5))}get endMs(){return this.end.getTime()}constructor(start,end,viewMode){this.viewMode=viewMode,this.start=start,this.end=end}pxToMs(px){return px*this.msPerPx}msToPx(ms){return ms/this.msPerPx}dateToPx(date){let span=date.getTime()-this.startMs;return this.msToPx(span)}pxToDate(px){let ms=this.pxToMs(px);return new Date(this.startMs+ms)}pxForTimeSpan(start,end){let span=end.getTime()-start.getTime();return this.msToPx(span)}};var import_dayjs8=__toESM(require_dayjs_min(),1);var WcGantt=class extends s3{constructor(){super(...arguments);this.__widthUpdated=!1;this.__timeScaleEndGapDays=7;this.___lastMovement=0;this.interactionReady=!1;this.scrollReady=!1}updateSettings(){let rowHeight=parseFloat(getComputedStyle(this).getPropertyValue("--gantt-layout-row-height")),scaleHeight=parseFloat(getComputedStyle(this).getPropertyValue("--gantt-layout-time-scale-height")),barHeight=parseFloat(getComputedStyle(this).getPropertyValue("--gantt-layout-bar-height")),lineWidth=parseFloat(getComputedStyle(this).getPropertyValue("--gantt-layout-line-width")),defaultOpts=new WcGanttSettings;this.settings={...defaultOpts,scaleHeight,rowHeight,barHeight,lineWidth,labelsWidth:0,width:0,height:0,...this.options},this.schedule=new Schedule(this.settings.dataDate,this.items??[],this.dependencies??[]),this.baselineItems.length>0&&(this.baselineSchedule=new Schedule(this.settings.baselineDate,this.baselineItems??[],this.baselineDependencies??[])),this.updateLabelsWidth(),this.updateWidth(),this.settings.height=this.schedule.items.length*this.settings.rowHeight;let timeScaleStart=Math.min(this.settings.startDate.getTime(),this.settings.dataDate.getTime(),this.settings.baselineDate.getTime(),this.schedule.startDate.getTime(),this.baselineSchedule?.startDate.getTime()??Number.MAX_VALUE);this.timeScale=new TimeScale((0,import_dayjs8.default)(timeScaleStart).subtract(5,"days").toDate(),this.schedule.endDate,this.settings.timeScaleMode),this.updateTimeScaleEndDate(),this.setupInteractions()}updateWidth(){let bcr=this.getBoundingClientRect();if(this.settings.width=bcr.width-this.settings.labelsWidth-(bcr.right-bcr.width)-bcr.left,this.timeScale){let dataBasedWidth=(this.schedule.durationDays+this.__timeScaleEndGapDays)*this.timeScale.pxPerDay;dataBasedWidth>this.settings.width&&(this.settings.width=dataBasedWidth)}this.__widthUpdated=!0}updateTimeScaleEndDate(){let maxDateBasedOnWidth=new TimeScale(this.settings.startDate,this.settings.startDate,this.settings.timeScaleMode).pxToDate(this.settings.width).setHours(0,0,0,0),maxDateBasedOnData=(0,import_dayjs8.default)(this.schedule.endDate).add(7,"days").toDate().getTime();this.timeScale.end=new Date(new Date(Math.max(maxDateBasedOnData,maxDateBasedOnWidth)))}get suppressClick(){return new Date().getTime()-this.___lastMovement<100}async setupInteractions(){this.interactionReady||(await this.updateComplete,configureAddLink.bind(this)(),configureResizeItem.bind(this)(),this.interactionReady=!0)}configureDependencyClick(){let links=this.shadowRoot.getElementById("gantt").querySelectorAll(".link"),handler=e6=>{let linkEl=e6.currentTarget,dep={predecessor:linkEl.dataset.source,successor:linkEl.dataset.target,type:linkEl.dataset.linkType},ev=new CustomEvent("dependency-click",{detail:dep});this.dispatchEvent(ev)};for(let l3 of links)l3.onclick=handler}get timeScaleElement(){return this.__timeScaleEl||(this.__timeScaleEl=this.renderRoot.querySelector(".time-scale")),this.__timeScaleEl}get timeScaleMarginElement(){return this.__timeScaleMarginEl||(this.__timeScaleMarginEl=this.renderRoot.querySelector(".time-scale-margin")),this.__timeScaleMarginEl}get ganttElement(){return this.__ganttEl||(this.__ganttEl=this.renderRoot.querySelector(".gantt")),this.__ganttEl}updated(){if(this.__widthUpdated=!1,!this.items||this.items.length===0||this.scrollReady)return;let el=this.renderRoot.querySelector("slot[data-labels]").assignedElements({flatten:!0})[0];if(!el)return;let config={childList:!0,subtree:!0,attributes:!0,characterData:!0},updateLabelsWidth=()=>{this.updateLabelsWidth(),this.updateWidth(),this.requestUpdate()},obs=new MutationObserver(updateLabelsWidth);obs.observe(el,config),el.shadowRoot&&obs.observe(el.shadowRoot,config),updateLabelsWidth(),this.scrollReady=!0}updateLabelsWidth(){let el=this.renderRoot.querySelector("slot[data-labels]")?.assignedElements({flatten:!0})[0];if(!el)return;let r7=el.getBoundingClientRect(),w2=Math.round((r7.width+Number.EPSILON)*10)/10;this.settings.labelsWidth=w2,this.updateWidth(),this.updateTimeScaleEndDate();let ganttV=this.renderRoot.querySelector(".gantt-v");this.timeScaleMarginElement.style.width=w2+"px",this.timeScaleElement.style.marginRight=ganttV.offsetWidth-ganttV.clientWidth+"px"}willUpdate(_changedProperties){(_changedProperties.has("items")||_changedProperties.has("dependencies")||_changedProperties.has("options"))&&this.updateSettings(),this.settings&&(this.settings.showCriticalPath&&this.schedule.updateCriticalPath(),this.updateTimeScaleEndDate(),this.updateComplete.then(()=>{this.configureDependencyClick()}))}render(){if(!this.items||this.items.length===0)return"No data";let labels=this.settings.showLabels?x`
    ...
    `,layoutCss,controlsCss,linkLineCss,barCss],__decorateClass([n3({type:Object,attribute:!1})],WcGantt.prototype,"options",2),__decorateClass([n3({type:Array,attribute:!1})],WcGantt.prototype,"items",2),__decorateClass([n3({type:Array,attribute:!1})],WcGantt.prototype,"dependencies",2),__decorateClass([n3({type:Array,attribute:!1})],WcGantt.prototype,"baselineItems",2),__decorateClass([n3({type:Array,attribute:!1})],WcGantt.prototype,"baselineDependencies",2),WcGantt=__decorateClass([t("wc-gantt")],WcGantt);export{Item,Schedule,WcGantt,WcGanttSettings};
